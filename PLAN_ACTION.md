...
- [ ] **4.2 Avant toute soumission ATN** — changer l'ID `magicthreads-b@xulforum.org` (domaine non possédé, ID **définitif** après publication) ; ajouter `icons` au manifest ; inclure LICENSE dans le XPI (`build.sh:35-46`) ; rédiger la justification des deux Experiment APIs ; extraction de version robuste dans `build.sh:14`.
